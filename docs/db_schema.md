...
基于方案 A (Electron + Python + SQLite)，为了支持 Phase 1 (Immersive) 和 Phase 2 (Knowledge)，我们需要设计一套以**内容**和**学习数据**为核心的 Schema。

**关键原则**：
1.  **UUID 主键**：所有表强制使用 UUID，为未来多端同步 (Phase 4) 铺路。
2.  **内容与进度分离**：视频元数据 (`MediaSource`) 与用户的学习行为 (`ReviewLog`) 分开。

---

## 🏗️ 核心对象 (Core Entities)

1.  **MediaSource (媒体源)**
    *   代表一个视频或音频文件。
    *   存储元数据（标题、时长、本地路径、封面图）。
2.  **SubtitleSegment (字幕片段)**
    *   代表媒体中的一句话。
    *   这是**核心交互单元**。点击它触发播放跳转；AI 针对它生成语法分析。
3.  **VocabularyItem (生词项)**
    *   代表用户收藏的一个单词或短语。
    *   **关键设计**：不仅存单词本身，还要存**来源上下文**（即出自哪句话），这是语言学习的关键。
4.  **ReviewLog (复习记录)**
    *   代表 SRS（间隔重复）的复习历史。
    *   记录“每一次”复习的时间、评分、下次复习时间。
5.  **UserSettings (用户设置)**
    *   单例表，存储全局偏好（如目标语言、默认倍速、API Key）。
...
## 🐍 Data Models (Python/SQLModel 伪代码)
...
class MediaSource(BaseModel, table=True):
    title: str
    file_path: str  # 本地绝对路径 或 URL
    cover_image: Optional[str] = None
    duration: float = 0.0
    language: str = "en"  # 原文语言
...
class VocabularyItem(BaseModel, table=True):
    ...
    word: str
    definition: str  # 用户可编辑的释义
    
    # 上下文快照 (即使原视频删了，这里也要保留例句)
...
1.  **`SubtitleSegment` 作为枢纽**：
    *   用户不只是在“背单词”，而是在“通过上下文背单词”。
    *   所有的 `VocabularyItem` 都通过 `segment_id` 强关联到**某一句具体的字幕**。
    *   这样在复习时，我们可以直接**播放那句话的原声**，而不只是显示冷冰冰的文字。

2.  **`sentence_snapshot` (快照 design)**：
    *   在 `VocabularyItem` 中，我额外增加了一个 `snapshot` 字段。
    *   虽然我们关联了 Segment，但如果未来用户删除了那个视频（Segment 也没了），生词本里的单词**不应该消失**。
    *   所以我们在创建生词那一刻，把例句**复制一份**存进生词表。这是一种**反范式 (Denormalization)** 设计，为了数据安全性和独立性。

3.  **SRS 字段预埋**：
    *   即使 Phase 1 我们不做复习功能，表结构里已经把 `ease_factor`, `interval`, `next_review_at` 留好了。这让未来的 Phase 2 升级无需修改数据库结构。
